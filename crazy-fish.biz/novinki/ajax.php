...
{
    $items = GetIBlockSectionList($iblock, 0, Array("sort"=>"asc"));
    while($arItem = $items->GetNext())
    {
        $arFilter = Array("IBLOCK_ID"=>$iblock, "SECTION_ID" => $arItem['ID'], "INCLUDE_SUBSECTIONS" => 'Y');
        $arSelect = Array("PROPERTY_NEW_STOCK");
        $res = CIBlockElement::GetList(Array("SORT"=>"ASC"), $arFilter,false, false, $arSelect);
        while($ob = $res->GetNextElement()){
            $arFields = $ob->GetFields();
            if($arFields['PROPERTY_NEW_STOCK_VALUE'] && !in_array($arItem["NAME"], $sectList)){
                $sectList[$arItem['ID']] = $arItem["NAME"];
            }
        }
    }
} ?>
<div class="row">
    <div class="col-lg-12 novinka-btns">
        <? $old = array("Crazy Fish", "CRAZY FISH");
           $new = array("", "");
        foreach ($sectList as $key => $item){?>
            <div class="catalog_novinka_item<?if($key==$_POST['section']){?> active<?}?>" data-section="<?=$key?>"><?echo str_replace($old, $new, $item);?></div>
        <?}?>
    </div>
    <div class="col-lg-12 text-center novinka-btns-more">
        <span class="novinka-bmore">Развернуть</span>
    </div>
</div>
<?
...
<script>
    $(document).ready(function () {
        let table = $('.novinka-btns');
        $('.novinka-bmore').click(function () {
            if(table.hasClass('active')){
                table.removeClass('active');
                $('.novinka-bmore').text('Развернуть');
            }else{
                table.addClass('active');
                $('.novinka-bmore').text('Свернуть');
            }

        });
        $(".catalog_novinka_item").click(function () {
            let section = $(this).data('section');
            let block = $(".novinki-page");
            $.ajax({
                type: "POST",
                url: '/novinki/ajax.php',
